...
if (isNodeJS) {
	...
	platform.TextEncoder = util.TextEncoder;
	platform.TextDecoder = util.TextDecoder;
} else {
	let outputBuf = "";
	platform.fs = {
	...
	platform.TextEncoder = TextEncoder;
	platform.TextDecoder = TextDecoder;
}

const encoder = new platform.TextEncoder("utf-8");
const decoder = new platform.TextDecoder("utf-8");
...
export class Go {
	...
	constructor() {
		...
		const timeOrigin = Date.now() - platform.performance.now();
		this.importObject = {
			go: {
				...
				"runtime.wasmWrite": (sp) => {
					...
					platform.fs.writeSync(fd, new Uint8Array(this._inst.exports.mem.buffer, p, n));
				},

				// func nanotime() int64
				"runtime.nanotime": (sp) => {
					setInt64(sp + 8, (timeOrigin + platform.performance.now()) * 1000000);
				},
				...
				"runtime.getRandomData": (sp) => {
					platform.crypto.getRandomValues(loadSlice(sp + 8));
				},
				...
				"syscall/js.valueInstanceOf": (sp) => {
					mem().setUint8(sp + 24, loadValue(sp + 8) instanceof loadValue(sp + 16) as any);
				},
